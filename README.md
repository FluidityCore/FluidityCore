# ¡Hola! Soy Luis David Mostacero Cieza 👋

Desarrollador enfocado en soluciones robustas y escalables. Actualmente me desempeño como asistente en hardware y software, lo que me da una visión integral de la tecnología, desde los componentes físicos hasta el código.

## Sobre mí
- 💻 Actualmente trabajando en proyectos de innovación y tecnología.
- 📚 Expandiendo mis horizontes hacia el **Frontend** y el ecosistema de **Node.js**.

## Mi Stack Tecnológico

### Backend & Core
![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Spring Boot](https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white)

### En aprendizaje activo (Learning Path)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white)
![NestJS](https://img.shields.io/badge/nestjs-%23E0234E.svg?style=for-the-badge&logo=nestjs&logoColor=white)

### Herramientas
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)

## Metas para este año
- [ ] Dominar el desarrollo Frontend con **TypeScript**.
- [ ] Construir APIs robustas utilizando **Nest.js**.

## Estadísticas de GitHub

Aquí puedes ver mi actividad reciente y el progreso de mis repositorios:

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=FluidityCore&show_icons=true&theme=radical&hide_border=true&count_private=true" alt="Estadísticas de GitHub" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=FluidityCore&layout=compact&theme=radical&hide_border=true" alt="Lenguajes más usados" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=FluidityCore&theme=radical&hide_border=true" alt="Racha de Commits" />
</p>
---
"El orden en el código refleja el orden en las ideas."
